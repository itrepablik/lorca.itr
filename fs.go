...
func (fs *fs) Open(name string) (http.File, error) {
	...
	b, ok := assets[name]
	if !ok {
		// Modified by ITRepablik, create a new 'html' folder name outside of 'www' assets directory to avoid
		// 'go generate' and slowness during the development time to speed up the build process.
		// Afterward, at 'gen.go', include the 'html' folder when your application is ready for build and deployment.
		htmlFile := "./html/"+name
		...
		return &file{name: name, size: len(html), Reader: bytes.NewReader(html)}, nil
	}
